...
- [ ] **CONTRATO-03**: Consolidar `AGENT-CONTRACT.md` a v1.1 como fuente única del contrato
  ...
  - **📐 Medición del 2026-08-02 (corrige la redacción anterior de este ticket).** Dos barridos **independientes** con las funciones reales compiladas — uno de 217.728 combinaciones y otro de 86.016 hecho por separado para contrastarlo:
    - **La afirmación previa "el pool plano nunca subreporta" era FALSA.** En un **0,3–1,2 %** de los casos (según el espacio enumerado) la fórmula del ADR da **+1 punto** sobre la viva, por redondeo independiente de `fe_raw`/`be_raw` frente a redondeo conjunto. La magnitud es **siempre exactamente 1** y ningún caso cambia de nivel, así que la conclusión práctica se sostiene — pero la premisa, tal como estaba escrita, no.
    - **El impacto real es grande y asimétrico:** **~35 %** de las combinaciones cambia de nivel de riesgo (34,58 % y 36,14 % en cada barrido), y el **100 %** de esos cambios son **alto → medio** en ambos. Cero en la dirección contraria. Cablear el ADR haría al motor estrictamente **menos** sensible al riesgo.
    - **Delta máximo: 50 puntos** (coincide en ambos barridos). **La causa NO es exclusiva de los datos sensibles**, como decía la redacción anterior: el pilar BE carga `c_base` + sus penalizadores + **todo** el sub-panel DevOps, así que satura en 50 mucho antes que FE, cuyos 4 penalizadores suman 45 como máximo. Reproducido con `personal_general` (CO: c_base 40 + 35 BE + 25 DevOps → `be_raw` 100 → `be_score` 50, frente a 100 del pool plano). Con `sensitive` basta `c_base` = 80 para saturar, pero es un caso particular de la misma mecánica.
    ...
    - ⚠️ **Los porcentajes son estimaciones sobre enumeraciones arbitrarias del espacio de inputs, no propiedades del motor.** No citarlos con precisión de dos decimales. Lo robusto y reproducido de forma independiente: la dirección (siempre alto → medio), el delta máximo (50) y la existencia del contraejemplo de +1.
  - **Recomendación con evidencia: alinear el ADR y la documentación al motor vivo**, no el motor al ADR. Si aun así se decidiera cablear el ADR, `escalation_required` debe rediseñarse **en el mismo cambio** (gate explícito por categoría de dato), o ese ~35 % de casos que dejan de escalar queda sin ningún disparador de revisión humana. Ver `architecture/AGENT-CONTRACT-V1.1-DESIGN.md` §D7.
